...
The toolbar shows the version, e.g. `Intel overlay v1.2.2`, plus `modern` or `legacy`. If you do not see that number, Chrome still has an old build loaded.
...
- **Range** — two presets, shortcut **Alt+5**:
  - **Reach** — Alt-drag sets combat size. Drag the combat ring to resize it later. When radar or sight sits on that same ring, use the **R** / **S** dots (or the short arc toward them) to resize sensors; the rest of the ring still resizes combat. Radar and sight start at the origin. Drag their hub anywhere **inside** the combat circle; the hub cannot leave that ring, but the radar/sight circles may extend past it. Drag the origin to move everything. Labels show radius in km.
  - **Radar+Sight** — Alt-drag sets radar size; sight starts smaller. Drag the origin to move; drag the **R** / **S** dots or their rings to resize.
...
Listing copy, original icons, and a pack script live in `store/`. Run `.\store\pack.ps1` to build `dist/con-intel-overlay-1.2.2.zip`. See `store/LISTING.md`.

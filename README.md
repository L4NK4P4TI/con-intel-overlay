...
The toolbar shows the version, e.g. `Intel overlay v1.2.1`, plus `modern` or `legacy`. If you do not see that number, Chrome still has an old build loaded.
...
- **Range** — two presets, shortcut **Alt+5**:
  - **Reach** — Alt-drag sets combat size (then it locks). Radar and sight start at the origin. Drag their hub anywhere **inside** the combat circle; the hub cannot leave that ring, but the radar/sight circles may extend past it. Drag the origin to move everything; drag the **R** or **S** dots to resize. Labels show radius in km.
...
Listing copy, original icons, and a pack script live in `store/`. Run `.\store\pack.ps1` to build `dist/con-intel-overlay-1.2.1.zip`. See `store/LISTING.md`.
